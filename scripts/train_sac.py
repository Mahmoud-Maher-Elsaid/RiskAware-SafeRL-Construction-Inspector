...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/training/sac.yaml"))
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/final_submission/sac"))
    ...
    environment = DummyVecEnv(
        [lambda: flatten_research_environment(config, continuous_actions=True)]
    )
    ...
    if resume_checkpoint.exists() and settings.get("resume", True):
        model = SAC.load(resume_checkpoint, env=environment, device=settings["device"])
        ...
        model = SAC(
            ...
            train_freq=int(settings["train_frequency"]),
    ...
    metadata = {
        ...
        "continuous_action_adapter": "five equal bins over [-1, 1]",
