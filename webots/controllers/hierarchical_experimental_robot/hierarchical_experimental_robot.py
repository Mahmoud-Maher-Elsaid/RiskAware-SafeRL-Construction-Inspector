...
def primitive_to_wheels(primitive: int) -> tuple[float, float]:
    """Map planner primitives to bounded differential-drive commands.

    Primitive indices are the training contract: forward, reverse, left,
    right, and stop.  Left and right intentionally have opposite wheel signs;
    neither command is generated by adding noise to the motors.
    """
    # The local planner's grid uses row/column cardinal moves while this robot
    # drives along +X.  Primitive 3 (column +1) is therefore forward; the
    # remaining cardinal moves are mapped to distinct reverse/turn commands.
    # Webots wheel polarity is negative in the +X forward direction.
    mapping = {
        0: (1.5, -1.5),
        1: (-1.5, 1.5),
        2: (0.85, 0.85),
    ...
    if primitive not in mapping:
        raise ValueError(f"Invalid planner primitive: {primitive}")
...
def map_from_perception(
    ...
    *,
    ...
    # DistanceSensor reports its maximum value when no obstacle is detected.
    sensor_strength = 1.0 - np.clip(
        np.asarray(obstacle_values, dtype=np.float32) / 1000.0, 0.0, 1.0
    )
    ...
    with (
        ...
        while RUN_MODE == "until_closed" or decision_index < DECISIONS:
            ...
            obstacle_values = np.asarray(
                [float(sensor.getValue()) for sensor in obstacle_sensors], dtype=np.float32
            )
            ...
            if before_target is not None:
                ...
                if (
                    target_last_distance is not None
                    and target_distance >= target_last_distance - 0.02
                ):
